...
export class MyLoader {
    ...
    constructor() {
        URL.customFormat = MyLoader.formatUrl
    }
    ...
    static formatUrl(url: string) {
        ...
        if (MyLoader.isWebp && StringUtil.endsWithAny(url, "png", "jpg")) url += ".webp"
    ...
    load(url: string | Array<string | LoadRes>, complete?: Handler, progress?: Handler, type?: string, priority = 1, cache = true, group?: string, ignoreCache = false, useWorkerLoader = false) {
        ...
        if (!ignoreCache && content != null) {
            ...
            let resInfo: ResInfo = this._infoPool.length ? this._infoPool.pop() : new ResInfo()
            resInfo.url = url
    ...
    private loadAssets(arr: Array<string | LoadRes>, complete: Handler, progress: Handler, type: string, priority: number, cache: boolean, group: string) {
        ...
        function loadComplete(item: any, content: any = null) {
            loadedCount++
            item.progress = 1
    ...
    private singleCompleteHandler(resInfo: ResInfo, content: any = null) {
        if (content == null) {
            if (this.baseUrls) {
                resInfo.useIndex++
