...
export class HtmlWindow extends fgui.Window implements IRecord {
    ...
    static get inst(): HtmlWindow {
        if (this._instance == null) this._instance = new HtmlWindow
    ...
    showTip(url: string, full = false, closeHandler?: ParamHandler) {
        ...
        if (Player.inst.isWeb) {
            ...
            let webElement: any = Browser.getElementById("webId")
            if (webElement == null) {
...
//			    Log.debug(Laya.stage.width, Render.canvas.width, Render._mainCanvas.width)
    ...
    /** 修正宽高 */
    _syncInputTransform() {
        let frameBox: any = Browser.getElementById("webId")
        if (frameBox == null) return
        let style: any = frameBox.style
        let transform: any = Utils.getTransformRelativeToWindow(this.displayObject, 0, 0)
