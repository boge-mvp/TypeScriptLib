...
//		Log.debug("visibilityChange="+Laya.stage.isVisibility)
    ...
    openGame(config: string, code = -1) {
        ...
        if (config) config = StringUtil.replace(config, "ColdCup", "GoldCup")
        // 处理房间号
        if (code < 0) code = ConfigUtils.gameCode(config)
        // 处理房间名字
...
//		}
        ...
        if (content == null) {
            MyLoader.loader.load(gameResJS,
    ...
    private loadJsComplete() {
        ...
        LoadingWindow.inst.show(1, this.getString(LibStr.LOADING))
...
//                // 放到下一帧去播放  不然 进入需要旋转的游戏 渲染跟不上
    ...
    private loadResErrorHandler() {
        ...
        if (Player.inst.urlParam.isJumpPage()) {
            if (!Render.isConchApp)
                JSUtils.openModal(this.getString(LibStr.NET_ERROR))
            JSUtils.gameClose()
