...
//		Log.debug("visibilityChange="+Laya.stage.isVisibility)
    ...
    openGame(config: string, code = -1) {
        ...
        // 处理房间名字
        if (code > 0 || config == null) config = ConfigUtils.gameNameCanonical(code)
        // 处理房间号
        if (code <= 0 && config) code = ConfigUtils.gameCode(config)
...
//		}
    ...
    private loadGameResComplete(content: string) {
        ...
        MathKit.loadScript(content, true, Render.isConchApp ? null : "gameRes.js")
        this.loadGameJs()
    }
...
//                // 放到下一帧去播放  不然 进入需要旋转的游戏 渲染跟不上
    ...
    closeGame() {
        ...
        if (MyLoader.checkBaseUrl) URL.basePath = MyLoader.checkBaseUrl()[0]
